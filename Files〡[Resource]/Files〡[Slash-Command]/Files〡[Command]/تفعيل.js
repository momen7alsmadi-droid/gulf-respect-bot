...
export default {
    name: "تفعيل",
    description: "تفعيل عضو جديد وتغيير اسمه",
    type: 1,
    options: [
        { name: "العضو", description: "العضو المراد تفعيله", type: ApplicationCommandOptionType.User, required: true },
        { name: "الاسم", description: "الاسم الجديد", type: ApplicationCommandOptionType.String, required: true }
    ],
    run: async (Client, Message) => {
        await Message.deferReply({ flags: 64 });
        try {
            const { default: origCmd } = await import("../../Files〡[Command-Handler]/Files〡[Admin]/CD〡[Tf3el].js");

        let fakeContent = "Client.Prefix" + "تفعيل";
