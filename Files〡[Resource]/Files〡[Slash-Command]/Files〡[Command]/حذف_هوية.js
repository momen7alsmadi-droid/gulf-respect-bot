...
export default {
    name: "حذف_هوية",
    description: "حذف الهوية الوطنية لعضو",
    type: 1,
    options: [
        { name: "العضو", description: "العضو", type: ApplicationCommandOptionType.User, required: true }
    ],
    run: async (Client, Message) => {
        await Message.deferReply({ flags: 64 });
        try {
            const { default: origCmd } = await import("../../Files〡[Command-Handler]/Files〡[Admin]/CD〡[Delete-ID].js");

        let fakeContent = "Client.Prefix" + "حذف-هوية";
