...
import { MainTicket, TicketT2dem } from '../Files〡[Config]/Files〡[Config].js';
...
يمكنك التواصل مع اللأونرية في طلب اونر لتافصيل اكثر __**`, flags: 64
 });
 const Data = db.get(`Ticket-${Message.guild.id}-${Message.user.id}-T2demAdmin`);
 ...
 {
 id: TicketT2dem.Support,
 allow: [PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.SendMessages]
 }
 ]
 })
 db.set(`Ticket-${Message.guild.id}-${Message.user.id}-T2demAdmin`, { Channel: ChannelTicket.id, Member: Message.user.id, Guild: Message.guild.id });
 const StringSelect = new StringSelectMenuBuilder({ customId: 'Select-Ticket-4', placeholder: 'خيارات التكت' });
 StringSelect.addOptions([
 { label: 'إسـتـلام الـتـكـت', value: 'ReceiveTicket-4' },
 { label: 'تـرك الإسـتـلام', value: 'LeaveReception-4' },
 { label: 'قـفـل الـتـكـت', value: 'CloseTicket-4' },
...
5 - مـدة تـواجُـدك فـي الـدسـكـورد بـالـسـاعـات :

6 - عـدد الـرحـلات الـتـي سـيـتـم دخـولـهـا بـالـيـوم :

— أتـعـهـد أنـا كـاتـب الـبـيـان أن جـمـيـع الـمـعـلـومـات الـتـي فـي الأعـلـى صـحـيـحـة وسـألـتـزم فـيـهـا .

~~-<@&${TicketT2dem.Support}> ~~ ${Message.user}**__`
 await ChannelTicket.send({ content: `${Content}`, components: [{ type: 1, components: [StringSelect] }] }).then(async () => {
 ...
 switch (Message.customId) {
 case 'Select-Ticket-4': {
 switch (Message.values[0]) {
 case 'ReceiveTicket-4': {
 ...
 if (!Member.roles.cache.some((Role) => [TicketT2dem.Support].includes(Role.id))) return await Message.reply({ content: `**لا يمكنك استلام التكت**`, flags: 64 });
 await Message.channel.permissionOverwrites.edit(Member.id, { ViewChannel: true, SendMessages: true });
 await Message.channel.permissionOverwrites.edit(TicketT2dem.Support, { ViewChannel: true, SendMessages: false });
 ...
 await Message.channel.permissionOverwrites.edit(TicketT2dem.Support, { ViewChannel: true, SendMessages: true });
 DB〡AdminPoint.findByIdAndUpdate({ _id: Message.user.id }, { $inc: { Point: -3 } }, { upsert: true, new: true }).catch(() => {});
 ...
 const ChannelLog = Message.guild.channels.cache.get(TicketT2dem.ChannelLog);
 ...
 case 'AddPerson-4': {
 const Member = await Message.guild.members.fetch(Message.user.id, { force: true, cache: true });
 if (!Member.roles.cache.some((Role) => [TicketT2dem.Support, TicketT2dem.Management].includes(Role.id))) return await Message.reply({ content: `**لا يمكنك استلام التكت**`, flags: 64 });
 const TextInput = new TextInputBuilder({ customId: 'AddPerson-4', label: 'إضافة شخص', placeholder: 'ادخل ايدي الشخص الذي تريد اضافته', style: 1 });
 const ActionRow = new ActionRowBuilder({ components: [TextInput] });
 const Modal = new ModalBuilder({ customId: 'AddPerson-4', title: 'إضافة شخص', components: [ActionRow] });
 await Message.showModal(Modal);
 } break;
 case 'RemovePerson-4': {
 const Member = await Message.guild.members.fetch(Message.user.id, { force: true, cache: true });
 if (!Member.roles.cache.some((Role) => [TicketT2dem.Support, TicketT2dem.Management].includes(Role.id))) return await Message.reply({ content: `**لا يمكنك استلام التكت**`, flags: 64 });
 const TextInput = new TextInputBuilder({ customId: 'RemovePerson-4', label: 'طرد شخص', placeholder: 'ادخل ايدي الشخص الذي تريد طرده', style: 1 });
 const ActionRow = new ActionRowBuilder({ components: [TextInput] });
 const Modal = new ModalBuilder({ customId: 'RemovePerson-4', title: 'طرد شخص', components: [ActionRow] });
 await Message.showModal(Modal);
 } break;
 }
 }
 ...
 switch (Message.customId) {
 case 'AddPerson-4': {
 const Member = Message.guild.members.cache.get(Message.fields.getTextInputValue('AddPerson-4'));
 ...
 case 'RemovePerson-4': {
 const Member = Message.guild.members.cache.get(Message.fields.getTextInputValue('RemovePerson-4'));
