...
export default {
    name: "نداء",
    description: "إرسال نداء خاص لعضو",
    type: 1,
    options: [
        { name: "العضو", description: "الشخص المطلوب", type: ApplicationCommandOptionType.User, required: true },
        { name: "السبب", description: "سبب النداء", type: ApplicationCommandOptionType.String, required: true }
    ],
    run: async (Client, Message) => {
        await Message.deferReply({ flags: 64 });
        try {
            const { default: origCmd } = await import("../../Files〡[Command-Handler]/Files〡[Admin]/CD〡[come].js");

        let fakeContent = "Client.Prefix" + "نداء";
