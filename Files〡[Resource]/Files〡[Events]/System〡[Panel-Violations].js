...
export default async function (Client, Message) {
    switch (Message.customId) {
        ...
        case 'Violations〡Member〡Modal': {
            ...
            StringSelectMenu.addOptions(Violations.Violations.map((violation, index) => ({ label: violation.Violation, description: `${violation.Description}`, emoji: violation.Emoji, value: `${index + 1}-$${Member.user.id}` })));
