...
export default async function (Client, Message) {
    if (Message.isButton()) {
        switch (Message.customId) {
            case 'Tickets-2SmHelp': {
                const Data = db.get(`Ticket-${Message.guild.id}-${Message.user.id}-2SmHelp`);
                ...
                const ChannelTicket = await Message.guild.channels.create({
                    ...
                    permissionOverwrites: [
                        ...
                        {
                            id: Tickets2Sm.Help.Support,
                            allow: [PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.SendMessages]
                        },
                        {
                            id: Tickets2Sm.Help.Management,
                            allow: [PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.SendMessages]
                        },
                        {
                            id: Tickets2Sm.Help.ToManagement,
                            allow: [PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.SendMessages]
                        }
                    ]
                })
                db.set(`Ticket-${Message.guild.id}-${Message.user.id}-2SmHelp`, { Channel: ChannelTicket.id, Member: Message.user.id, Guild: Message.guild.id });
...
— مـرحـبـآ بـك عـزيـزي الـعـضـو فـي قـسـم المساعدة . 
...
\`\`\`               𝗪𝗢𝗟𝗙 𝗖𝗜𝗧𝗬  \`\`\``);
                ...
                const StringSelect = new StringSelectMenuBuilder({ customId: 'Select-Ticket-3', placeholder: 'خيارات التكت' });
                StringSelect.addOptions([
                    { label: 'إسـتـلام الـتـكـت', value: 'ReceiveTicket-3' },
                    { label: 'تـرك الإسـتـلام', value: 'LeaveReception-3' },
                    { label: 'قـفـل الـتـكـت', value: 'CloseTicket-3' },
                    { label: 'إضـافـة شـخـص', value: 'AddPerson-3' },
                    { label: 'طـرد شـخـص', value: 'RemovePerson-3' },
                ]);
                await ChannelTicket.send({ content: `${Message.user} | <@&${Tickets2Sm.Help.Support}>`, embeds: [Embed], components: [{ type: 1, components: [StringSelect] }], files: [Attachment] }).then(async () => {
        ...
        switch (Message.customId) {
            case 'Select-Ticket-3': {
                switch (Message.values[0]) {
                    case 'ReceiveTicket-3': {
                        ...
                        if (!Member.roles.cache.some((Role) => [Tickets2Sm.Help.Support].includes(Role.id))) return await Message.reply({ content: `**لا يمكنك استلام التكت**`, flags: 64 });
                        await Message.channel.permissionOverwrites.edit(Member.id, { ViewChannel: true, SendMessages: true });
                        await Message.channel.permissionOverwrites.edit(Tickets2Sm.Help.Support, { ViewChannel: false, SendMessages: true });
                    ...
                    case 'LeaveReception-3': {
                        ...
                        await Message.channel.permissionOverwrites.edit(Tickets2Sm.Help.Support, { ViewChannel: true, SendMessages: true });
                        await DB〡AdminPoint.findByIdAndUpdate({ _id: Message.user.id }, { $inc: { Point: -2 } }, { upsert: true, new: true });
                    ...
                    case 'CloseTicket-3': {
                        ...
                        const ChannelLog = Message.guild.channels.cache.get(Tickets2Sm.Help.ChannelLog);
                    ...
                    case 'AddPerson-3': {
                        const Member = await Message.guild.members.fetch(Message.user.id, { force: true, cache: true });
                        if (!Member.roles.cache.some((Role) => [Tickets2Sm.Help.Support].includes(Role.id))) return await Message.reply({ content: `**لا يمكنك استلام التكت**`, flags: 64 });
                        const TextInput = new TextInputBuilder({ customId: 'AddPerson-3', label: 'إضافة شخص', placeholder: 'ادخل ايدي الشخص الذي تريد اضافته', style: 1 });
                        const ActionRow = new ActionRowBuilder({ components: [TextInput] });
                        const Modal = new ModalBuilder({ customId: 'AddPerson-3', title: 'إضافة شخص', components: [ActionRow] });
                        await Message.showModal(Modal);
                    } break;
                    case 'RemovePerson-3': {
                        const Member = await Message.guild.members.fetch(Message.user.id, { force: true, cache: true });
                        if (!Member.roles.cache.some((Role) => [Tickets2Sm.Help.Support].includes(Role.id))) return await Message.reply({ content: `**لا يمكنك استلام التكت**`, flags: 64 });
                        const TextInput = new TextInputBuilder({ customId: 'RemovePerson-3', label: 'طرد شخص', placeholder: 'ادخل ايدي الشخص الذي تريد طرده', style: 1 });
                        const ActionRow = new ActionRowBuilder({ components: [TextInput] });
                        const Modal = new ModalBuilder({ customId: 'RemovePerson-3', title: 'طرد شخص', components: [ActionRow] });
                        await Message.showModal(Modal);
                    } break;
                }
            }
        ...
        switch (Message.customId) {
            case 'AddPerson-3': {
                const Member = Message.guild.members.cache.get(Message.fields.getTextInputValue('AddPerson-3'));
            ...
            case 'RemovePerson-3': {
                const Member = Message.guild.members.cache.get(Message.fields.getTextInputValue('RemovePerson-3'));
