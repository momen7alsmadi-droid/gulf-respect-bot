...
export default {
    name: "مالك",
    description: "إدارة الملاك المشاركين",
    type: 1,
    options: [
        { name: "إجراء", description: "الإجراء", type: ApplicationCommandOptionType.String, required: true, choices: [{ name: "عرض الملاك", value: "عرض" }, { name: "إضافة مالك", value: "اضافة" }, { name: "حذف مالك", value: "حذف" }] },
        { name: "العضو", description: "العضو (للإضافة/الحذف)", type: ApplicationCommandOptionType.User, required: false }
    ],
    run: async (Client, Message) => {
        await Message.deferReply({ flags: 64 });
        try {
            const { default: origCmd } = await import("../../Files〡[Command-Handler]/Files〡[Admin]/CD〡[OwnerManager].js");

        let fakeContent = "Client.Prefix" + "مالك";
