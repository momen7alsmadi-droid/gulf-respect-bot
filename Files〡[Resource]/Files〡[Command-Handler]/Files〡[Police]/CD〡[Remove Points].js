...
export default {
    name: 'إزالة-نقاط',
    description: "إزالة نقاط للعضو",
    /**
    * @param { import('discord.js').Client } Client
    * @param { import('discord.js').Message } Message
    */
    run: async (Client, Message) => {
        if (!Message.member.roles.cache.some(Role => Police.RemovePoint.includes(Role.id))) return Message.reply({ content: `**ليس لديك الصلاحية لإزالة النقاط**` });
